...
- **짝 맞추기는 세션이 아니다 (ADR-22)**: 홈의 세 번째 카드지만 암송 모드가 아니라 재인(알아보기) 게임이다. 하드 드릴과 같은 범위 시트로 범위를 고르고, 진행은 `AppData.pair`에 저장되어 이어할 수 있다(`sessions`에는 넣지 않는다). **`seen`·`stats`·하드드릴 부채는 절대 건드리지 않는다** — 재인 신호를 회상 기록에 섞으면 안 된다. 점수·노화 규칙은 `lib/pair-game.ts`와 ADR-22.
